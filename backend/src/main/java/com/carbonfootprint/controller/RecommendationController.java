...
public class RecommendationController {
    ...
    @PostMapping("/adopt")
    public ResponseEntity<ApiResponse<Void>> adoptRecommendation(
            Authentication authentication,
            @RequestBody java.util.Map<String, String> payload) {
        String recommendationText = payload.get("recommendation");
        String email = authentication.getName();
        log.info("REST request to adopt recommendation for user: {}", email);
        recommendationService.adoptRecommendation(email, recommendationText);
        return ResponseEntity.ok(ApiResponse.success(null, "Recommendation adopted successfully"));
    }
}
