...
const ZoomableAreaChart = ({ data = [], dataKey = "emissions", xAxisKey = "label", color = "#0ea5e9" }) => {
    ...
    <div className="w-full h-[350px] flex flex-col select-none">
      ...
      <div className="flex-1">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} onMouseDown={(e) => e && setRefAreaLeft(e.activeLabel)} onMouseMove={(e) => e && refAreaLeft && setRefAreaRight(e.activeLabel)} onMouseUp={zoom} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
            <defs>
            ...
            <XAxis dataKey={xAxisKey} domain={[left, right]} allowDataOverflow tick={{ fill: '#64748b', fontSize: 12 }} tickLine={false} axisLine={false} minTickGap={20} />
            <YAxis domain={[bottom, top]} allowDataOverflow tick={{ fill: '#64748b', fontSize: 12 }} tickLine={false} axisLine={false} />
            <Tooltip contentStyle={{ borderRadius: '6px', border: '1px solid #e2e8f0', boxShadow: 'none' }} itemStyle={{ color: '#0f172a', fontWeight: '500' }} formatter={(value) => [Number(value).toLocaleString() + ' kg', 'Emissions']} />
            <Area type="monotone" dataKey={dataKey} stroke={color} strokeWidth={0} fillOpacity={1} fill="url(#colorArea)" activeDot={{ r: 4, strokeWidth: 0, fill: color }} />
...
export default React.memo(ZoomableAreaChart);
