...
const Navbar = ({ onOpenSidebar }) => {
  ...
  React.useEffect(() => {
    // Add Google Translate Script dynamically if it doesn't exist
        ...
        <div className="flex items-center">
          {isProfileIncomplete && (
            <div className="hidden sm:flex items-center gap-1.5 mr-3 px-2.5 py-1 bg-rose-50 border border-rose-100 rounded-full animate-pulse">
              ...
              <span className="text-[10px] font-medium text-rose-600 uppercase tracking-wider">{t('navbar.setup_required')}</span>
            </div>
          )}
          <div className="relative">
            <button 
              onClick={() => setIsNotificationsOpen(!isNotificationsOpen)}
              className="relative p-1.5 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-md transition-colors focus:outline-none"
            >
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" /></svg>
              {unreadCount > 0 && (
                <span className="absolute -top-1 -right-1 flex h-4 min-w-[16px] items-center justify-center px-1 rounded-full bg-emerald-500 text-[10px] font-bold text-white shadow-sm ring-2 ring-white">
        ...
        <div className="hidden sm:flex items-center gap-3">
          ...
          {points.currentStreak > 0 && (
            <div 
              ...
              title={`${t('navbar.longest_streak')}: ${points.longestStreak} days`}
            >
          ...
          <button
            ...
            title={`${t('navbar.level')}: ${points.currentLevel}`}
          >
